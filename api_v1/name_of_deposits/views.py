...
router = APIRouter(prefix="/name_of_deposits", tags=['reference_books', 'name_of_deposits'])
...
@router.get("/{device_id}", response_model=NameOfDeposits)
async def get_name_of_deposit(deposit_id: int, session: AsyncSession = Depends(db_helper.session_dependency),):
    device = await crud.get_name_of_deposit(session=session, deposit_id=deposit_id)
    if device is None:
        raise HTTPException(
